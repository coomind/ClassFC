...
function Members({ members }) {
  ...
  const positionList = ["ALL", "GK", "DF", "MF", "FW"];
  ...
  const sortedMembers = [...filteredMembers].sort((a, b) => {
    ...
    if (a.role === "Vice-Captain") {
      return -1;
    }

    if (b.role === "Vice-Captain") {
      return 1;
    }
    ...
    <div className="container page-section members-page">
      ...
      <div className="members-filter">
        {positionList.map((position) => (
          <button
            key={position}
            className={filter === position ? "pos-filter-btn active" : "pos-filter-btn"}
          ...
          >
            ...
            <span className="pos-filter-count">
              {position === "ALL"
                ? members.length
                : members.filter((member) => member.position === position).length}
            </span>
          </button>
        ))}
      </div>
      ...
      {/* 선수 상세 모달 */}
      {selected && (
        <div className="member-modal-backdrop" onClick={closeMember}>
          <div
            className="member-modal-box"
            onClick={(event) => event.stopPropagation()}
          >
            <button className="member-modal-close" onClick={closeMember}>
              ×
            </button>
            ...
            <div className="member-modal-stats">
              ...
              {selected.position === "GK" ? (
                <div className="modal-stat">
                  <div className="modal-stat-num">
                    {selected.cleanSheets || 0}
                  </div>
